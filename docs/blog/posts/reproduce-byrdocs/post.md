...
- 本项目的最佳布署方法是使用 [Wrangler CLI](https://developers.cloudflare.com/workers/wrangler/)。它作为 `byrdocs` 仓库的依赖包，在 [准备环境](#准备环境) 一步中可以安装。
...
7. 填写环境变量。你需要准备以下两个文件：
    1. `.env` 定义了前端用到的环境变量，格式如下，请依实际情况自行更改。
    ```ini
    # 主站网址，也是你为主站设定的自定义域名，见下文第10步
    ...
    ```
    2. `.dev.vars` 定义了后端用到的环境变量，格式如下，请依实际情况自行更改。
    ```ini
    # 第6步得到的 D1 读写 Token
    CLOUDFLARE_D1_TOKEN=
...
```
10. 在 [Workers & Pages](https://dash.cloudflare.com/?to=/:account/workers-and-pages) 列表中，选中自己的 `byrdocs` 站点。找到 *Settings* -> *Build* ，点击 *Connect*，打开选单。选好你的 GitHub 仓库和分支，*Build command* 填 `pnpm run build`，*Deploy command* 填 `npx wrangler deploy`。完成之后点击 *Connect*。
11. 同样在 `byrdocs` 站点设置中，找到 *Domains & Routes*，点击 *+Add* -> *Custom domain*，填写你希望使用的域名。
...
- BYR Docs 的数据文件位于另一个 Cloudflare R2 存储桶内，有且只有 5 个：
    ...
    - `wiki.json` 记录 [维基真题](#维基真题) 的文件信息，以供后台处理。如果你无意搭建维基真题，可以删除不使用它。
...
```
3. [上传 GitHub Secrets](https://docs.github.com/en/actions/how-tos/write-workflows/choose-what-workflows-do/use-secrets)，以便应用于 GitHub Actions 当中。一共有 14 个 Secrets 需要上传：
    ...
    9. `BACKUP_ACCESS_KEY_ID` `BACKUP_SECRET_ACCESS_KEY` 是你在创建 R2 备份存储池的 Token 时记录下来的值。如果你的备份存储池和主存储池相同，只需使用第二步中相同的值即可。
    10. `BACKUP_FILE_BUCKET` 是你定义的资源文件备份存储桶名称，在这里是 `byrdocs-backup`。
...
BYR Docs Publish 是一个 Cloudflare Page，它的布署方法与主站有些不同。

### 所需环境

- 你需要拥有一个 [Cloudflare 账号](https://dash.cloudflare.com)。
- 你的本地开发环境需要安装 [Node.js](https://nodejs.org) 及 [pnpm](https://pnpm.io/)。
- 本项目的最佳布署方法是使用 [Wrangler CLI](https://developers.cloudflare.com/workers/wrangler/) 和 [Prisma](https://www.prisma.io/)。它们作为 `byrdocs-publish` 仓库的依赖包，在 [准备环境](#准备环境) 一步中可以安装。
...
4. 配置 D1 数据库。
    ...
    ```
    4. 为 `byrdocs-publish` 创建名为 `create_table` 的数据库迁移。然后，你将在 `migrations/` 文件夹下看到一个新的迁移文件 `migrations/0001_create_table.sql`。
    ```bash
    npx wrangler d1 migrations create byrdocs-publish create_table
    ```
    5. 现在向 `migrations/0001_create_table.sql` 中以 `prisma/schema.prisma` 生成文件内容。
    ```bash
    npx prisma migrate diff \
        --from-empty \
        --to-schema-datamodel ./prisma/schema.prisma \
        --script \
        --output ./migrations/0001_create_table.sql
    ```
    6. 应用 D1 数据库迁移。
    ```bash
...
2. 填写 App 的有关配置。（假设你的 BYR Docs Publish 站点域名为 `PUBLISH_URL`）
    1. *GitHub App Name* 和 *Homepage URL* 都可以任意填写。
    2. *Identifying and authorizing users*：
        1. *Callback URL* 填为 `PUBLISH_URL/callback/github`；如果你希望在本地运行和测试，还可以添加一个 `http://localhost:3000/callback/github`。
        2. 选中 *Expire user authorization tokens*。
        3. 不选中 *Request user authorization (OAuth) during installation* 和 *Enable Device Flow*。
    3. *Post installation*：
        1. *Setup URL* 填为 `PUBLISH_URL/bind`。
        2. 选中 *Redirect on update*。
    4. *Webhook*：
        1. *Webhook URL* 填为 `PUBLISH_URL/api/github/webhook`。
        2. *Secret* 请自行生成一个足够强的字符串填入，并记下它，作为 `WEBHOOK_SECRET`。
    5. *Permissions*：
...
以上需要你记录的量有五个：`APP_ID` `GITHUB_APP_PRIVATE_KEY` `GITHUB_CLIENT_ID` `GITHUB_CLIENT_SECRET` `WEBHOOK_SECRET`。请注意留存。

#### 准备仓库

1. 你可通过 Fork 或自行上传等方法，建立一个 `byrdocs-archive` 仓库。建议将该仓库置于一个你管理的组织账户下，作为**基准仓库**。包括你在内的所有人都要 Fork 该仓库来为其作出贡献。
2. 将[准备 GitHub App](准备-github-app) 步骤中你创建的 GitHub App 安装到基准仓库中。

#### 准备环境变量

你需要准备两个文件：`.env` 和 `.dev.vars`，格式如下：

```ini
# .env
# 依具体情况，自行更改以下项
NEXT_PUBLIC_SITE_BASE_URL="https://byrdocs.org"
NEXT_PUBLIC_DATA_BASE_URL="https://files.byrdocs.org"
NEXT_PUBLIC_PUBLISH_SITE_BASE_URL="https://publish.byrdocs.org"
NEXT_PUBLIC_DEV_SITE_BASE_URL="http://localhost:3000"
NEXT_PUBLIC_PUBLISH_APP_BASE_URL="https://github.com/apps/byrdocs-publish"
NEXT_PUBLIC_ARCHIVE_REPO_OWNER="byrdocs" # 基准仓库的所有者
NEXT_PUBLIC_ARCHIVE_REPO_NAME="byrdocs-archive" # 基准仓库的名称
NEXT_PUBLIC_ARCHIVE_REPO_URL="https://github.com/byrdocs/byrdocs-archive" # 基准仓库的
```

```ini
# .dev.vars
# 依具体情况，自行更改以下项
APP_ID= # 创建 GitHub App 时记下的值
GITHUB_APP_PRIVATE_KEY= # 创建 GitHub App 时记下的值
GITHUB_CLIENT_ID= # 创建 GitHub App 时记下的值
GITHUB_CLIENT_SECRET= # 创建 GitHub App 时记下的值
JWT_SECRET= # 自行生成一个
WEBHOOK_SECRET= # 创建 GitHub App 时记下的值
```

接下来，将 `.dev.vars` 中的运行时变量上传至 Cloudflare Secrets 中，使其在远端生效。
```bash
npx wrangler secret bulk .dev.vars
```

现在再运行 `pnpm run deploy` 即可正常使用 BYR Docs Publish。
